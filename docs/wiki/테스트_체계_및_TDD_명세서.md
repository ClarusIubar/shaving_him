# 🧪 [Test Hierarchy & TDD Specification] Shaving Him 테스트 체계서 - v1.14.0
...
1. **도메인 단위 테스트 (`tests/domain/*`)**: 순수 도메인 비즈니스 로직(점수 계산, 비트맵 연산, 세션 상태 머신, 스키마 검증, 타이머) 격리 검증.
2. **포트/어댑터 계약 테스트 (`tests/ports/*`, `tests/adapters/*`)**: LSP 원칙 준수 여부 및 추상 포트 미구현 에러 발생 검증.
3. **UI 컴포넌트 단위 테스트 (`tests/ui/*`)**: 뷰 컴포넌트, 브러시 컨트롤러, 입력 매니저, 커서 뷰, Web Audio 합성기 격리 검증.
...
- **`createMockCanvasElement()`**: `getContext('2d')`, 픽셀 데이터 조작, `toDataURL` 지원.
- **`setupGlobalDOM()`**: 글로벌 `document`, `window` 격리 주입 및 자동 원복 `teardown()` 헬퍼.

---

## 4. 테스트 스위트 전수 매트릭스 (150 Tests Matrix)
...
✔ SessionTimer - ticks interval and stops cleanly (SRP)
...
✔ ScoreCalculator - custom IScoringStrategy strategy pattern verification (OCP)
...
✔ CursorView - translate3d GPU positioning, font size scaling, and visibility toggling
...
✔ InputManager - Fail-Fast, binds sound toggle, brush buttons, keyboard shortcuts (1-4, R), destroy(), granular views injection (ISP)
...
src                           |        |          |         | 
 ...
 app                          |        |          |         | 
  ...
  game-orchestrator.js        | 100.00 |   100.00 |  100.00 | 
 ...
 domain                       |        |          |         | 
  ...
  session-timer.js            | 100.00 |   100.00 |  100.00 | 
 ...
 ui                           |        |          |         | 
  ...
  canvas-renderer.js          | 100.00 |    97.37 |  100.00 | 
  hud.js                      | 100.00 |    98.39 |  100.00 | 
  input-manager.js            | 100.00 |    98.21 |  100.00 | 
  particle-system.js          | 100.00 |   100.00 |  100.00 | 
  sound-effects.js            | 100.00 |    97.73 |  100.00 | 
 views                        |        |          |         | 
  cursor-view.js              | 100.00 |   100.00 |  100.00 | 
